...
static esp_err_t PID_handler(httpd_req_t* req) {
    ...
    Serial.println("\npid kp " + String(Kp_val) + " ki " + String(Ki_val) + " kd " + String(Kd_val));
    double Kp = atof(Kp_val);
    double Ki = atof(Ki_val);
    double Kd = atof(Kd_val);
    int res = 0;
...
static esp_err_t motorsCmd_handler(httpd_req_t* req) {
    char* buf;
    size_t buf_len;
    char rightValue[32] = { 0, };
    char leftValue[32] = { 0, };
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            if (httpd_query_key_value(buf, "left", leftValue, sizeof(leftValue)) == ESP_OK &&
                httpd_query_key_value(buf, "right", rightValue, sizeof(rightValue)) == ESP_OK) {
            }
            else {
                free(buf);
                httpd_resp_send_404(req);
    ...
    int right  = atoi(rightValue);
    int left = atoi(leftValue);
    int res = 0;

    appControl->move(right, left);
...
static esp_err_t index_handler(httpd_req_t* req) {
    ...
    return httpd_resp_send(req, (const char*)new_index_ov2640_html_gz, new_index_ov2640_html_gz_len);

}

void startCameraServer(Control* control) {
